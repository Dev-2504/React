...
{/* Product Add Pop up */}
    ...
    <div id='Add' className='h-screen w-[350px] translate-x-full absolute right-0 top-0 bg-[#30334e] z-30' style={{transition:"all 0.3s ease-In-Out"}}>
        ...
        <div className={isSidebar ? 'open':'close'}>
        ...
        <div className='h-auto flex flex-col px-[20px]'>
            ...
            <div className='h-[200px] w-full relative  rounded-[7px] mt-[15px] p-[10px] flex flex-col items-center justify-center border text-white border-white'>
            <input required type="file" placeholder='Product Brand Name' className='z-20 text-[0px] absolute top-0 left-0 w-full h-full rounded-[5px] px-[10px] mt-[15px] outline-none' onChange={Hello2}/>
            <img src={file} alt="" className='absolute max-h-[175px]'/>
            </div>
            ...
            <button className='px-[20px] py-[10px] border mt-[15px] ml-[15px] rounded-[7px] text-white border-white' onClick={abc}>Preview</button>
        </div>
    </div>


    <div id='preview' className="hidden z-50 absolute left-1/2 top-0 -translate-x-1/2 w-[600px] h-[350px] none p-[20px] border border-amber-50 bg-[#282a42] text-white">
    <div className='w-full flex items-center justify-between'>
        <p className='text-[25px] font-medium'>Preview</p>
        <svg onClick={handleClick}  xmlns="http://www.w3.org/2000/svg" width="30px" height="30px" viewBox="0 0 16 16"><path fill="none" stroke="#fff" strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="m11.25 4.75l-6.5 6.5m0-6.5l6.5 6.5"></path></svg>
    </div>
