...
export default function SignIn() {
  ...
  const handleSubmit = (e) => {
    ...
    if(data && Object.keys(data).length > 0)
    {
      ...
      {
        const notify = toast.success("Dashboard upar Redirect.")
        console.log("Dashboard")
        localStorage.setItem("auth",JSON.stringify(true))
      }
      else {
        const notify = toast.error("Password Was Inncorect.")
        console.log("Password Was Incorrect")
      }
    }
    else {
      const notify = toast.info("You're not registered. Please sign up")
      console.log("Email")
    }
  }
  ...
  return (
    // Design
    <div className="w-full min-h-screen max-h-max bg-[#686279] flex items-center" id="SignUp_Wrapper">
      <div className="w-[70%] min-h-[75%] max-h-max flex mx-[auto] bg-[#2c2638] py-[15px] pl-[15px] rounded-2xl">
        {/* Left */}
        <div className="w-[50%]" id="SignUp_Image"></div>
        {/* Right */}
        <div className="w-[50%] px-[120px] py-[50px]">
          <p className="text-5xl text-white text-center">Welcome Back</p>
          <p className="text-center mt-[8px] text-[#888]">Enter your email and password to access your account.</p>

          <form action="" onSubmit={handleSubmit} className="mt-[40px]">
            <input type="text" autoComplete='off' name='email' value={formdata.name} placeholder="Email" onChange={handleChange}  className="h-[50px] w-full mt-[20px] bg-[#3c364c] px-[15px] placeholder-[#77777] text-white rounded-md outline-none"/>
            <div className="h-[50px] mb-[10px] rounded-md flex items-center justify-between bg-[#3c364c] px-[15px] mt-[20px]">
            <input type={passwordshow ? "text" : "password"}  autoComplete='off' name='password' value={formdata.password} onChange={handleChange} placeholder="Enter your Password" className="w-[300px] placeholder-[#77777] text-white rounded-md outline-none"/>
            <svg onClick={() => setPasswordshow(!passwordshow)} className='cursor-poinater' xmlns="http://www.w3.org/2000/svg" width="25px" height="25px" viewBox="0 0 24 24"><g fill="none" stroke="#888" strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}><path d="M15 12a3 3 0 1 1-6 0a3 3 0 0 1 6 0"></path><path d="M2 12c1.6-4.097 5.336-7 10-7s8.4 2.903 10 7c-1.6 4.097-5.336 7-10 7s-8.4-2.903-10-7"></path></g></svg>
            </div>
            <Link className="ml-[10px] text-[#6d54b5] font-medium mt-[10px] cursor-pointer" to={"/forgetpassword"}>Forget Password</Link>
            <button type='submit' className="w-full h-[50px] mt-[40px] bg-[#6d54b7] cursor-pointer text-white text-[18px] rounded-md">Login</button>
          </form>
